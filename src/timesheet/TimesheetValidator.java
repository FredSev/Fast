...
public class TimesheetValidator {
    ...
    final static String ERROR_MSSAGE_MAX_HOURS_DAY = ", l'employé a déclaré plus de 24 heures"
            + " pour la journée.";
    ...
    private void validateMaxHoursPerDay() {
        for (Integer i = 0; i < SUNDAY; i++) {
            if (employee.getTempsUnJour(i) > MAX_HOURS_DAY) {
                errors.add(WEEK_DAYS_STRING[i] + ERROR_MSSAGE_MAX_HOURS_DAY);
            }
        }
    }
